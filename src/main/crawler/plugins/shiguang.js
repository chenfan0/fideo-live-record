...
const log = debug('fideo-crawler-shiguang')

function getRoomIdByUrl(url) {
  return new URL(url).pathname.split('/')[1]
}

async function baseGetShiGuangLiveUrlsPlugin(roomUrl, others = {}) {
  ...
  for (const match of matches) {
    ...
    const roomInfo = (
      await request(
        `https://wap.rengzu.com/api/ui/room/v1.0.0/live.ashx?promoters=0&roomidx=${roomId}&currentUrl=${roomUrl}`,
        {
          headers: {
            cookie,
            'User-Agent': MOBILE_USER_AGENT
